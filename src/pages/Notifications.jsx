  ...
  useEffect(() => {
    const fetchPendingRequests = async () => {
      ...
      try {
        ...
        const response = await axiosInstance.get(`/api/friends/pending/${userId}`);
          ...
          {pendingRequests.length > 0 ? (
            pendingRequests.map((request) => (
              <div key={request.id} className="notification-card">
                ...
                <button onClick={() => handleAcceptRequest(request.user.id)}>Accept</button>
                <button onClick={() => handleDeclineRequest(request.user.id)}>Decline</button>
              </div>
            ))
          ) : (
